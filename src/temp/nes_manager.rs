...
impl FrameSender {
    pub fn new(thread_com: &ThreadCom) -> Self {
        ...
        let thread_handle: JoinHandle<()> = new_named_thread("nes-render", move || {
            loop {
...
impl Drop for FrameSender {
    fn drop(&mut self) {
        if thread::panicking() {
            ...
            if result.is_err() {
                panic!(
                    "Failed to send Exit message to 'nes-render' thread! FrameSender's thread channel was disconnected!"
                );
            }
...
impl Drop for NESMessenger {
    fn drop(&mut self) {
        if thread::panicking() {
            ...
            if result.is_err() {
                panic!(
                    "Failed to send Exit message to 'nes-messenger' thread! NESMessenger's thread channel was disconnected!"
                );
            }
            self.thread_handle.take().unwrap().join().unwrap();
        }
...
impl NESManager {
    pub fn new(texture_handle: egui::TextureHandle) -> Self {
        NESManager {
            nes_thread: None,
            ...
            frametimes_index: 0,
            screen_texture: texture_handle,
        }
    }

    pub fn start_nes(&mut self, rom_path: PathBuf) {
        assert!(
        ...
        self.nes_thread = Some(new_named_thread("nes", move || {
            ...
            let rom_bytes: Vec<u8> = std::fs::read(rom_path).unwrap();
    ...
    pub fn handle_nes_messages(&mut self) {
        ...
        for message in messages.iter() {
            match message {
                ThreadMsg::NewFrame(frametime, frame) => {
                    ...
                    self.screen_texture.set(
                        ColorImage::from_rgba_unmultiplied([256, 240], &pixels),
                    ...
                    self.frametimes_index =
                        (self.frametimes_index + 1) % self.frametimes.capacity();
                    self.frametime =
                        self.frametimes.iter().sum::<f64>() / self.frametimes.len() as f64;
                ...
                _ => error!(
                    "NESManager received a '{:?}' message, which it cannot proccess. Ignoring message",
                    message
                ),
            }
        }
