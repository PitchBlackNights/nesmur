...
pub fn format_mem(memory: &Vec<u8>, start_addr: u16, end_addr: u16) -> String {
    let mut buffer: String = String::from("ADDR   00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F  ASCII\n-------------------------------------------------------------------------\n");
        ...
        for byte_num in 0x00..0x0F + 1 {
            if shift_line_num + byte_num < start_addr || shift_line_num + byte_num > end_addr {
                ...
                let ascii_char: String = if 0x20 >= ascii_byte || ascii_byte >= 0x7E {
                    format!("{}", 0x2E as char) // Char: `.`
                } else {
                    format!("{}", ascii_byte as char)
                };

                line += format!("{}", ascii_char).as_str();
            }
        }
        buffer += (line + "\n").as_str();
    }
