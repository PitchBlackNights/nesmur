...
impl ROM {
    pub fn new(raw: &[u8]) -> Result<ROM, String> {
        ...
        let ines_ver: u8 = if (raw[7] >> 2) & 0b0000_0011 == 0 {
            1
        } else {
            2
        };
        ...
        let screen_mirroring: Mirroring =
            match (alt_nametable_layout, vertical_mirroring) {
                (true, _) => return Err(
                    "This ROM uses the mapper specific \"Alternative Nametable Layout\" feature!"
                        .to_string(),
                ),
        ...
        if ines_ver == 1 {
            match raw[7] & 0b0000_0011 {
                ...
                _ => {
                    return Err(
                        "ROM provides invalid console type (0x3) for iNES 1.0 format!".to_string(),
                    );
                }
            };
            ...
            Ok(ROM {
                ...
                mapper,
                submapper: 0,
                region: region.unwrap_or(ROMRegion::NTSC),
                prg_rom,
            ...
            match raw[7] & 0b0000_0011 {
                ...
                0x1 => return Err("The Nintendo VS. System is not supported!".to_string()),
                0x2 => return Err("The Nintendo Playchoice 10 is not supported!".to_string()),
                0x3 => match raw[13] & 0b0000_1111 {
                    0x0 => {}
                    0x1 => return Err("The Nintendo VS. System is not supported!".to_string()),
                    0x2 => return Err("The Nintendo Playchoice 10 is not supported!".to_string()),
                    0x3 => {
                        return Err(
                            "Famiclones with Decimal Mode support are not supported!".to_string()
                        );
                    }
                    0x4 => {
                        return Err(
                            "The EPSM module and/or Plug-through cartridges are not supported!"
                                .to_string(),
                        );
                    }
                    ...
                    _ => {
                        return Err(format!(
                            "ROM provides invalid extended console type ({:#3X}) for iNES 2.0 format!",
                            raw[13] & 0b0000_1111
                        ));
                    }
                },
            ...
            if raw[14] & 0b0000_0011 != 0 {
                return Err(
                    "iNES 2.0 bundled \"Miscellaneous\" ROMs are not supported!".to_string()
                );
            }
            ...
            let mapper: u16 = ((raw[8] as u16 & 0b0000_1111) << 8)
                | (raw[7] & 0b1111_0000) as u16
                | (raw[6] >> 4) as u16;
            let submapper: u8 = raw[8] >> 4;

            let prg_rom_size: usize = match raw[9] & 0b0000_1111 {
                0x0..0xF => {
                    (((raw[9] as usize & 0b0000_1111) << 8) | raw[4] as usize) * PRG_ROM_PAGE_SIZE
                }
                0xF => {
                    let multiplier: u8 = raw[4] & 0b0000_0011;
                    let exponent: u8 = raw[4] >> 2;
                    2u32.saturating_pow(exponent as u32)
                        .saturating_mul(multiplier as u32 * 2 + 1) as usize
                }
                _ => panic!("This shouldn't happen!"),
            };
            let prg_rom_start: usize = 16 + if uses_trainer { 512 } else { 0 };
            let prg_rom: Vec<u8> = raw[prg_rom_start..(prg_rom_start + prg_rom_size)].to_vec();
            let prg_ram_size_index: u8 = match uses_bat_mem {
                false => raw[10] & 0b0000_1111,
                true => raw[10] >> 4,
            };
            let prg_ram_size: Option<usize> = match prg_ram_size_index {
                0 => None,
                _ => Some(64usize << (prg_ram_size_index) as usize),
            };
            ...
            let chr_rom_start: usize = prg_rom_start + prg_rom_size;
            let chr_rom: Vec<u8> = raw[chr_rom_start..(chr_rom_start + chr_rom_size)].to_vec();
            let chr_ram_size_index: u8 = match uses_bat_mem {
                false => raw[11] & 0b0000_1111,
                true => raw[11] >> 4,
            };
            let chr_ram_size: Option<usize> = match chr_ram_size_index {
                0 => None,
                _ => Some(64usize << (chr_ram_size_index) as usize),
            };
